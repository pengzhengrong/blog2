	...
	Public function page_blog() {
	 ...
	 Public function category() {
		...
		$limit = $this->page($count ,$pageSize, $url);
	 	...
	 	$rest = M('blog')->cache(true,CACHE_TIME)->field(array('id','title','content','created'))->where("status=0 AND cat_id in ($ids)")->order('created desc')->limit($limit)->select();
	 	$this->rest = $rest;
	 ...
	 Private function get_next_prev($id , $type='') {
	 	...
	 	switch ($type) {
	 		...
	 		$rest = M('blog')->cache(true,CACHE_TIME)->field(array('id','title'))->where("status=0 AND cat_id in ($ids)")->order('created desc')->select();
	 		break;
	 		default:
